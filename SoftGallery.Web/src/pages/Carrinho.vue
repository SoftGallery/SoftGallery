        ...
        <div class="max-w-6xl mx-auto px-4">
            <div class="grid sm:grid-cols-1 xl:grid-cols-3 gap-8">
                <div v-if="carrinho.length >= 1"
                    class=" bg-gray-100 shadow-lg  rounded-3xl border border-white/20 col-span-2  overflow-hidden ">
                    <div v-for="p in carrinho" :key="p.id" class="p-8 hover:bg-gray-50/50 transition-all duration-200">
                        <div class="flex items-center space-x-6">
                            <div
                                class="min-h-10 bg-gray-100 p-4 shadow-lg rounded-2xl w-30 h-30 overflow-hidden flex-shrink-0">
                                <img class="rounded-2xl h-full w-full object-cover" :src="p.thumbnail" alt="">
                            </div>
                            <div class="min-h-10 overflow-hidden">
                                ...
                                <div class="flex space-x-2 items-center">
                                    <span
                                        class="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-500 to-violet-700">${{
                                        p.precoDesconto }}</span>
                                    <span
                                        class="rounded-full py-1 px-2 text-xs bg-green-100 text-green-900 font-semibold">Em
                                        estoque</span>
                                </div>
                            </div>
                            <div class="min-h-10 flex items-center p-2 rounded-2xl bg-gray-100 space-x-3">
                                <button @click="diminuirQuantidade(p)"
                                    class="cursor-pointer py-1 px-2 bg-gray-200/50 rounded-lg hover:scale-110 hover:bg-rose-300 transform transition duration-200 hover:shadow">
                                    <i class="fa-solid fa-minus text-rose-500"></i>
                                </button>
                                <span class="text-lg font-bold text-center text-slate-900">{{ p.quantidade }}</span>
                                <button @click="aumentarQuantidade(p)"
                                    class="cursor-pointer py-1 px-2 bg-gray-200/50 rounded-lg hover:scale-110 hover:bg-emerald-200 transform transition duration-200 hover:shadow">
                            ...
                            <div class="min-h-10 flex flex-col items-center">
                                ...
                                <button @click="deleteProduto(p.id)"
                                    class="flex justify-between gap-3 bg-red-100 p-2 rounded-lg font-medium items-center text-red-400 text-sm hover:scale-110 transform transition duration-200 hover:shadow cursor-pointer">
                ...
                <div v-else
                    ...
                    <div class="p-16 flex flex-col items-center">
                        ...
                        <div class="flex gap-4 mt-12 justify-center">
                            <router-link to="/"
                                class="px-8 py-4 rounded-2xl bg-gradient-to-r from-purple-500 to-violet-700 text-white font-bold shadow-lg hover:scale-110 transform transition duration-200 hover:shadow-lg">
                                <i class="fa-solid fa-magnifying-glass text-white"></i> Descubra Produtos
                            </router-link>
                            <router-link
                                class="px-8 py-4 rounded-2xl border-2 border-gray-200 shadow-lg font-bold hover:scale-110 transform transition duration-200 hover:shadow-lg">
                                <i class="fa-regular fa-heart"></i> Ver Favoritos
                            </router-link>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col">
                    <div class="bg-gray-100 rounded-3xl shadow-lg border border-white/20 p-10 w-full backdrop-blur-sm">
                        <div class="flex items-center space-x-3 mb-6">
                            <div class="bg-gray-200/50 py-2 px-3 rounded-xl">
                                <i
                                    class="fa-solid fa-calculator text-3xl bg-gradient-to-r from-purple-500 to-violet-700 bg-clip-text text-transparent"></i>
                            </div>
                        ...
                        <div v-else class="flex justify-between text-gray-600 mb-4">
                            <span class="font-medium">Frete</span>
                        ...
                        <div class="pt-3 border-t border-gray-200">
                            <div class="flex justify-between">
                                ...
                                <span v-if="carrinho.length >= 1"
                                    class="text-3xl font-bold bg-gradient-to-r from-purple-500 to-violet-700 bg-clip-text text-transparent">
                                    ${{ (valorTotal + 29.99).toFixed(2) }}
                                </span>
                                <span v-else
                                    class="text-3xl font-bold bg-gradient-to-r from-purple-500 to-violet-700 bg-clip-text text-transparent">
                                    $0.00
                        ...
                        <router-link to="/agradecimento"
                            class="w-full mt-4 p-3 bg-black rounded-xl hover:scale-105 transition transform duration-200 cursor-pointer hover:shadow-xl block text-center"
